...
class GeneradorPDF:
    """ Clase para generar y regresar bytes de archivos PDF. """
    def __init__(self, conn):
        self.conn = conn
    
    def _generarOrdenCompra(self, idx: int):
...
class ImpresoraPDF:
    ...
    def __init__(self, parent: QWidget = None):
        ...
        self.generadorPDF = GeneradorPDF(self.conn)
...
class ImpresoraOrdenes(ImpresoraPDF):
    ...
    def imprimirOrdenCompra(self, idx: int):
        if not self.printer:
            return
        data = self.generadorPDF._generarOrdenCompra(idx)
...
class ImpresoraTickets(ImpresoraPDF):
    ...
    def imprimirTicketCompra(self, idx):
        ...
        data = self.generadorPDF._generarTicketPDF(
            idx, productos, vendedor, fechaCreacion, pagado, metodo)
        self.enviarAImpresora(data)
    ...
    def imprimirTicketPresupuesto(self, productos, vendedor):
        ...
        data = self.generadorPDF._generarTicketPDF(
            0, productos, vendedor, QDateTime.currentDateTime(), 0., None)
        self.enviarAImpresora(data)
    ...
    def imprimirCorteCaja(self, caja: Caja, user: Usuario):
        ...
        if not self.printer:
            return
        data = self.generadorPDF._generarCortePDF(caja, user)
        self.enviarAImpresora(data)
