...
class DatabaseManager:
    ...
    def __handle_err(self, err: FirebirdError):
        ...
        body = (self.__class__.__name__ + ' {', f'{gdscode=}', txt, '}')
...
class ManejadorProductos(DatabaseManager):
    ...
    def eliminarPrecios(self, id_productos: int):
        ...
        return all(
            self.execute(query.format(tabla), param)
            for tabla in ['productos_intervalos', 'productos_gran_formato']
        )
...
class ManejadorVentas(DatabaseManager):
    ...
    def insertarPago(
        ...
        ''',
            (id_ventas, id_metodo, monto, recibido, id_usuarios),
            commit=commit,
        )
